#!/usr/bin/env bash
# Darklabs Git Wizard — interactif, sûr, sans merge automatique
# - Initialisation possible si .git absent (git init, branche main)
# - Configuration du remote origin si manquant (prompt HTTPS/SSH)
...
ensure_git_repo() {
  if [ ! -d .git ]; then
    warn "Aucun dépôt Git (.git) détecté ici : $(pwd)"
    if confirm "Initialiser un dépôt Git dans ce dossier ?" "y"; then
      run_cmd "git init"
      run_cmd "git branch -M main"
      info "✅ Dépôt initialisé (branche 'main')."
    else
      err "Interrompu : place-toi dans le dossier du projet puis relance."
      exit 1
    fi
  fi
}
...
configure_remote_origin_if_needed() {
  local remote_url
  remote_url="$(git remote get-url origin 2>/dev/null || true)"
  if [ -z "$remote_url" ]; then
    warn "Aucun remote 'origin' configuré."
    say "Configurer 'origin' pour le dépôt GitHub 'darklabs'"
    info "Exemples :"
    info "  HTTPS : https://github.com/<TonCompte>/darklabs.git"
    info "  SSH   : git@github.com:<TonCompte>/darklabs.git"
    printf "URL du remote origin : "
    read -r remote_url || true
    if [ -z "${remote_url:-}" ]; then
      err "URL vide. Impossible de continuer."
      exit 1
    fi
    run_cmd "git remote add origin \"$remote_url\""
    info "✅ Remote origin configuré : $remote_url"
  else
...
say "0) Pré-vol"
run_cmd "git rev-parse --abbrev-ref HEAD || echo '(aucune branche encore)'"
run_cmd "git remote -v"

say "1) Se caler sur main à jour"
# crée 'main' si nécessaire
if ! git show-ref --verify --quiet refs/heads/main; then
  warn "Branche 'main' absente localement — création."
...
assert_clean_tree_or_offer_fix
run_cmd "git fetch origin || true"
if git ls-remote --exit-code --heads origin main >/dev/null 2>&1; then
  run_cmd "git pull --ff-only || true"
else
  warn "Branche 'origin/main' introuvable (repo distant peut-être vide)."
fi
...
if git ls-remote --exit-code --heads origin main >/dev/null 2>&1; then
  ...
  warn "origin/main n'existe pas encore (repo distant vide ?). Rebase sauté."
fi
print_pr_link "$BR"
